...
| --- | --- | --- | --- | --- | --- |
| Stable macOS capture flow | Log capture start, selected source, window visibility state, permission state, image byte size, image dimensions, blank-frame validation result, save path, and failure reason. Logs must not include screenshot pixels. | Add unit coverage for capture result validation, blank-image detection, source filtering, source grouping, and capture metadata normalization. | Re-run capture smoke tests for current screen, full screen, browser/page window, terminal/app window, and permission-denied behavior. Verify Debugr does not appear in its own capture unless explicitly selected. | Manually capture the visible desktop, a browser, Terminal, and a multi-window app. Confirm the saved image visually matches the actual screen and is not empty. | No blank screenshots, no app-window hijack during crop, no proactive session window opening during capture, and capture completes within the target latency budget. |
